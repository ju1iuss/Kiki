...
export function Step4Signup() {
  ...
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === 'SIGNED_IN' && session?.user) {
        ...
        // Save onboarding images after signup (with retry logic)
        const saved = await saveOnboardingImages()
        if (!saved) {
          console.warn('Failed to save images immediately, will retry later')
        }
        nextStep()
      }
    })
  ...
  }, [supabase, updateData, nextStep, email])
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/callback?redirect=/onboarding`,
        },
      })
      ...
      if (signUpError && signUpError.message.includes('already registered')) {
        ...
        if (signInError) {
          if (signInError.message.includes('Invalid login credentials')) {
            setError('An account with this email already exists. Please use the correct password.')
          } else {
            throw signInError
          }
          return
      ...
      <div className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-gray-600 text-sm">
            Sign up to see your mockups and start creating
          </p>
        </div>
        ...
        {!showEmailForm ? (
          <>
            ...
            <Button
              ...
              disabled={loading}
              className="w-full bg-white text-black hover:bg-gray-100 border border-gray-300 font-marlinsoft h-12 transition-all duration-300 ease-out transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
              {loading ? (
                <>
            ...
            <Button
              ...
              disabled={loading}
              className="w-full bg-black text-white hover:bg-gray-800 border-0 font-marlinsoft h-12 transition-all duration-300 ease-out transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  id="email"
                  type="email"
                  placeholder="tiktok@tasy.ai"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  disabled={loading}
                  className="pl-10 bg-gray-50 border-gray-300 text-black h-12"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  disabled={loading}
                  minLength={6}
                  className="pl-10 pr-10 bg-gray-50 border-gray-300 text-black h-12"
                />
                <button
                  type="button"
        ...
        <div className="text-center">
          <p className="text-sm text-gray-500">
            ...
            <Link href="/sign-in" className="font-semibold text-gray-700 hover:text-black">
              Log in
            </Link>
          </p>
        </div>
